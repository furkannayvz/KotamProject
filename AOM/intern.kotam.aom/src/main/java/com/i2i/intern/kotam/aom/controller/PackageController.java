...
public class PackageController {
    ...
    public PackageController(PackageServiceOracle packageService, PackageSelectionService packageSelectionService) {
        this.packageService = packageService;
        this.packageSelectionService = packageSelectionService;
    }

    // Kullanicinin bir paketi secmesini saglar. Oracle -> VoltDB -> Hazelcast
    @PostMapping("/select")
    public ResponseEntity<String> selectPackage(@RequestParam String msisdn,
                                                @RequestParam Long packageId) {
        boolean success = packageSelectionService.processPackageSelection(msisdn, packageId);

        if (success) {
            return ResponseEntity.ok("Paket başarıyla seçildi ve veriler kaydedildi.");
        } else {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Paket seçimi başarısız oldu.");
        }
    }
