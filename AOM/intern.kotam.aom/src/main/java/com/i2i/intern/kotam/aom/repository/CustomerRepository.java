...
public class CustomerRepository {
    ...
    private boolean checkCustomerExists(String msisdn, String email, String tcNumber) throws SQLException, ClassNotFoundException {
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareStatement("SELECT COUNT(*) FROM CUSTOMER WHERE MSISDN = ? OR EMAIL = ? OR TC_NUMBER = ?");
            stmt.setString(1, msisdn);
    ...
    // Bu metodları mevcut CustomerRepository sınıfınıza ekleyin:

    /**
     * Find customer by MSISDN - PasswordResetService için
     */
    public Optional<Customer> findCustomerByMsisdn(String msisdn) throws SQLException, ClassNotFoundException {
        logger.debug("Finding customer by MSISDN: {}", msisdn);
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call GET_CUSTOMER_BY_MSISDN(?, ?)}");
            stmt.setString(1, msisdn);
            stmt.registerOutParameter(2, Types.REF_CURSOR);
            ...
            if (rs.next()) {
                Customer customer = mapCustomerFromResultSet(rs);
                logger.debug("Customer found successfully for MSISDN: {}", msisdn);
                return Optional.of(customer);
            } else {
                logger.debug("Customer not found for MSISDN: {}", msisdn);
                return Optional.empty();
            }

        } catch (Exception e) {
            logger.error("Error finding customer by MSISDN: {}", msisdn, e);
            throw new RuntimeException("Customer lookup failed: " + e.getMessage());
        } finally {
            closeResources(rs, stmt, connection);
        }
    }
    ...
    /**
     * Save customer - PasswordResetService için
     */
    public void saveCustomer(Customer customer) throws SQLException, ClassNotFoundException {
        logger.debug("Saving customer with ID: {}", customer.getCustomerId());
    ...
    /**
     * Insert new customer - private helper
     */
    private void insertNewCustomer(Customer customer) throws SQLException, ClassNotFoundException {
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call INSERT_CUSTOMER_FULL(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)}");

            stmt.setString(1, customer.getMsisdn());
            stmt.setString(2, customer.getName());
            stmt.setString(3, customer.getSurname());
            stmt.setString(4, customer.getEmail());
            stmt.setString(5, customer.getPassword());
            stmt.setString(6, customer.getTCNumber());
            stmt.setTimestamp(7, customer.getCreatedDate() != null ?
                    new Timestamp(customer.getCreatedDate().getTime()) : new Timestamp(System.currentTimeMillis()));
            stmt.setTimestamp(8, customer.getUpdatedDate() != null ?
                    new Timestamp(customer.getUpdatedDate().getTime()) : null);
            stmt.setBoolean(9, customer.getIsActive());
            stmt.registerOutParameter(10, Types.INTEGER);

            stmt.execute();
            customer.setCustomerId(stmt.getInt(10));

            logger.debug("Customer inserted successfully with ID: {}", customer.getCustomerId());
    ...
    /**
     * Update existing customer - private helper
     */
    private void updateExistingCustomer(Customer customer) throws SQLException, ClassNotFoundException {
        logger.debug("Updating customer with ID: {}", customer.getCustomerId());
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call UPDATE_CUSTOMER_FULL(?, ?, ?, ?, ?, ?, ?, ?, ?)}");

            stmt.setInt(1, customer.getCustomerId());
            stmt.setString(2, customer.getMsisdn());
            stmt.setString(3, customer.getName());
            stmt.setString(4, customer.getSurname());
            stmt.setString(5, customer.getEmail());
            stmt.setString(6, customer.getPassword());
            stmt.setString(7, customer.getTCNumber());
            stmt.setTimestamp(8, customer.getUpdatedDate() != null ?
                    new Timestamp(customer.getUpdatedDate().getTime()) : new Timestamp(System.currentTimeMillis()));
            stmt.setBoolean(9, customer.getIsActive());
