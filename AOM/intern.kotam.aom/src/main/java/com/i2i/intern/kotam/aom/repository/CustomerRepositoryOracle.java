...
public class CustomerRepositoryOracle implements CustomerRepositoryBase {
    ...
    public Optional<Customer> authenticateCustomer(String email, String password) {
        String sql = "{call AUTHENTICATE_CUSTOMER(?, ?, ?, ?)}"; // 4 parametre
        try (Connection conn = dataSource.getConnection();
             CallableStatement stmt = conn.prepareCall(sql)) {
            ...
            stmt.registerOutParameter(3, Types.VARCHAR); // p_msisdn OUT
            stmt.registerOutParameter(4, Types.INTEGER); // p_result OUT
            ...
            if (result == 1) {
                String msisdn = stmt.getString(3);
