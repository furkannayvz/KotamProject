...
public class  PackageRepository implements PackageRepositoryOracle {
    ...
    public Optional<PackageEntity> getPackageDetailsById(Long id) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            stmt.registerOutParameter(3, Types.DOUBLE);  // PRICE
            stmt.registerOutParameter(4, Types.BIGINT);  // MINUTES
            stmt.registerOutParameter(5, Types.BIGINT);  // SMS
            stmt.registerOutParameter(6, Types.BIGINT);  // DATA
            stmt.registerOutParameter(7, Types.INTEGER); // PERIOD

            stmt.execute();

            PackageEntity pkg = new PackageEntity();
            pkg.setId(id); // Giriş parametresiydi
            pkg.setName(stmt.getString(2));
            pkg.setPrice(stmt.getDouble(3));
            pkg.setMinutesQuota(stmt.getLong(4));
            pkg.setSmsQuota(stmt.getLong(5));
            pkg.setDataQuota(stmt.getLong(6));
            pkg.setPeriod(stmt.getInt(7));
