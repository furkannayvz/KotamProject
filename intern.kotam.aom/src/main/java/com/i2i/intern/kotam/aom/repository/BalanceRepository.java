...
public class BalanceRepository {
    ...
    public void updateBalanceDataInOracle(String msisdn, int leftData) {
        logger.info("Updating BAL_LEFT_DATA for MSISDN: {}", msisdn);
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call UPDATE_BALANCE_BY_MSISDN_DATA(?, ?)}");
            stmt.setString(1, msisdn);
            stmt.setInt(2, leftData);

            stmt.execute();
            logger.info("BAL_LEFT_DATA updated successfully for MSISDN: {}", msisdn);

        } catch (SQLException | ClassNotFoundException e) {
            logger.error("Failed to update BAL_LEFT_DATA for MSISDN: {}", msisdn, e);
            throw new RuntimeException("Oracle procedure execution failed: " + e.getMessage());
        } finally {
            dbManager.closeResources(stmt, connection);
        }
    }

    public void updateBalanceMinutesInOracle(String msisdn, int leftMinutes) {
        logger.info("Updating BAL_LEFT_MINUTES for MSISDN: {}", msisdn);
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call UPDATE_BALANCE_BY_MSISDN_MINUTES(?, ?)}");
            stmt.setString(1, msisdn);
            stmt.setInt(2, leftMinutes);

            stmt.execute();
            logger.info("BAL_LEFT_MINUTES updated successfully for MSISDN: {}", msisdn);

        } catch (SQLException | ClassNotFoundException e) {
            if (e instanceof SQLException && ((SQLException) e).getErrorCode() == 20001) {
                logger.warn("MSISDN bulunamadı: {}", msisdn);
                throw new DataNotFoundException("MSISDN bulunamadı.");
            }
            logger.error("BAL_LEFT_MINUTES güncellenirken hata oluştu", e);
            throw new RuntimeException("Oracle procedure execution failed: " + e.getMessage());
        } finally {
            dbManager.closeResources(stmt, connection);
        }
    ...
    public void updateBalanceSmsInOracle(String msisdn, int leftSms) {
        logger.info("Updating BAL_LEFT_SMS for MSISDN: {}", msisdn);
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call UPDATE_BALANCE_BY_MSISDN_SMS(?, ?)}");
            stmt.setString(1, msisdn);
            stmt.setInt(2, leftSms);

            stmt.execute();
            logger.info("BAL_LEFT_SMS updated successfully for MSISDN: {}", msisdn);
            ...
            logger.error("BAL_LEFT_SMS güncellenirken hata oluştu", e);
            throw new RuntimeException("Oracle procedure execution failed: " + e.getMessage());
        } finally {
            dbManager.closeResources(stmt, connection);
        }
    ...
    public void updateAllBalanceFieldsInOracle(String msisdn, int leftMinutes, int leftSms, int leftData) {
        logger.info("Updating all balance fields for MSISDN: {}", msisdn);
        ...
        try {
            connection = dbManager.getOracleConnection();
            stmt = connection.prepareCall("{call UPDATE_BALANCE_BY_MSISDN(?, ?, ?, ?)}");
            stmt.setString(1, msisdn);
            stmt.setInt(2, leftMinutes);
            stmt.setInt(3, leftSms);
            stmt.setInt(4, leftData);

            stmt.execute();
            logger.info("All balance fields updated successfully for MSISDN: {}", msisdn);
            ...
            logger.error("Tüm balance alanları güncellenirken hata oluştu", e);
            throw new RuntimeException("Oracle procedure execution failed: " + e.getMessage());
        } finally {
            dbManager.closeResources(stmt, connection);
        }
    ...
    public ResponseEntity<String> updateBalanceInOracle(BalanceUpdateRequest request) {
        updateAllBalanceFieldsInOracle(
                request.msisdn(),
                request.leftMinutes(),
                request.leftSms(),
                request.leftData()
