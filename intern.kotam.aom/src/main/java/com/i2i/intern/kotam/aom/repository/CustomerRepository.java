...
public class CustomerRepository {
    ...
    public void createCustomerInOracle(RegisterCustomerRequest request) throws ClassNotFoundException, SQLException {
        ...
        try {
            if (checkCustomerExists(request.msisdn(), request.email())) {
    ...
    public boolean authenticateCustomer(String email, String password)
        ...
        try (CallableStatement stmt = connection.prepareCall("{call AUTHENTICATE_CUSTOMER(?, ?, ?)}")) {
            stmt.setString(1, email);
            stmt.setString(2, password);
            stmt.registerOutParameter(3, Types.INTEGER); // 1: başarılı, 0: başarısız
            stmt.execute();
    ...
    public boolean checkCustomerExists(String msisdn, String email)
            throws SQLException, ClassNotFoundException {
        Connection connection = dbManager.getOracleConnection();
        String query = "SELECT COUNT(*) FROM CUSTOMER WHERE MSISDN = ? OR EMAIL = ?";
    ...
    public ResponseEntity<String> createCustomerInOracleSimple(RegisterCustomerRequest request)
            throws SQLException, ClassNotFoundException {

        logger.info("Creating customer in Oracle (simple) with MSISDN: {}", request.msisdn());
        ...
        Connection connection = dbManager.getOracleConnection();
        try (CallableStatement stmt = connection.prepareCall("{call INSERT_CUSTOMER_SIMPLE(?, ?, ?, ?, ?, ?)}")) {
            ...
            stmt.execute();
            logger.info("Customer created in Oracle (simple) successfully for MSISDN: {}", request.msisdn());
            return ResponseEntity.status(HttpStatus.CREATED).body("Customer created successfully in Oracle (simple)");
        } finally {
            dbManager.closeConnection(connection);
        }
    }
    ...
    private int insertCustomerInOracle(Connection connection, RegisterCustomerRequest request, String encodedPassword)
        ...
        try (CallableStatement stmt = connection.prepareCall("{call INSERT_CUSTOMER(?, ?, ?, ?, ?, ?)}")) {
            ...
            stmt.setString(6, request.tcNumber()); // National ID
            stmt.execute();
        }
    ...
    private int retrievePackageIdByName(Connection connection, String packageName) throws SQLException {
        try (CallableStatement stmt = connection.prepareCall("{call SELECT_PACKAGE_ID(?, ?)}")) {
            stmt.setString(1, packageName);             // P_PACKAGE_NAME (IN)
            stmt.registerOutParameter(2, Types.INTEGER); // P_PACKAGE_ID (OUT)
            stmt.execute();
